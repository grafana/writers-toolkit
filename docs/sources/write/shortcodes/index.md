...
The `code` shortcode provides the ability to show multiple snippets of code in different langauges. When a languge is selected, other code blocks on the page are toggled if the language is included. The selected langague is saved to browser and persists across naviagtions.
