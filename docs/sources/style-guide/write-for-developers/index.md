...
In Markdown, format a placeholder with an asterisk (\*) followed by a backtick (`) and an open angle bracket (<). 
Use a dash (-) in the placeholder name, if necessary. Close the placeholder name with an angle bracket (>), followed by a backtick and asterisk.
